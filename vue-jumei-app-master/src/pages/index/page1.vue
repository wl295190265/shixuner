  ...
  <div class="content-wrapper" ref="contentWrapper">
    <div>
      ...
      <section class="mixcart-list">
        ...
        <GoodsList :list="list" load="loading"></GoodsList>
      </section>
    </div>
  </div>
</div>
...
export default {
  ...
  created () {
    ...
    axios.get("/api/data").then(res => {
      ...
      this.list=res.data.activityLists
      // 异步的 属于放在这里面 等待他们只想完
      this.$nextTick( () => {
        this._initScroll()
      })
  ...
  methods: {
    ...
    load(){
    	if(this.loading==false){
    		...
    		this.list.push(this.list[0],this.list[1],this.list[2])
    		this.loading=false
    		},1000)
    ...
    _initScroll () {
      ...
      this.typeScroll.on('scrollEnd', (pos) => {
 				 console.log(pos.x + '~' + pos.y)
 				 console.log(this.$refs.contentWrapper.offsetHeight)
 				 if(-pos.y+50>=this.$refs.contentWrapper.offsetHeight){
 				 	this.load()
