      ...
      <div>
        <div class="user-block">
          <div class="user-login">
            <div class="user-logo">
              <img :src="userImg" @click="sheetVisible=true" alt="">
            </div>
            ...
            <div class="login-success" v-if="userLogin">
              ...
              <input class="autograph" ref="qianming"  disabled="disabled" type="text" v-model="qianming" name="" style="background: #ccc;" id="qianming" placeholder="我需要一个长势旺盛的个性签名" />
           			<label for="qianming" ><img @click="changeQianming" slot="icon" src="" alt="icon" width="24" height="24"></label>
            </div>
          </div>
          <ul class="user-info">
            ...
            <router-link to="/userinfo" tag="li" class="item"><span></span><p>个人资料</p></router-link>
          </ul>
          <div class="user-mask">
        ...
        <div class="order-fund-block">
          <div class="block-title">
            <span>买买买</span>
            <span @click="all()" class="block-title-nav" >查看全部订单</span>
          </div>
          ...
          <ul class="fund-content content">
            <li class="item" v-for="(item, index) in fundContent" @click="detail(item)" :key="index">
...
export default {
  data () {
    return {
    	userImg:"http://img02.sogoucdn.com/app/a/100520024/65883a8707e8c4171ac5a3f8b5d7ac28",
    	sheetVisible:false,
    	qianming:"",
    	actions:[
    		{
    			name:"拍照",
    			method:this.gotoPhoto
    		},
    		{
    			name:"相册选择",
    			method:function(){
  ...
  created () {
    ...
    if(storage["userImgUrl"]){
    	this.userImg=storage["userImgUrl"];
    }
    let username = storage["user"]
    username ? this.userLogin=username : this.userLogin=""
    
    let qianming = storage["qianming"]
    qianming ? this.qianming=qianming : this.qianming=""
    
  },
  methods: {
  	detail(item){
  		if(item.title=="红包"){
  			this.$router.push("/red_package")
  		}
  		
  	},
  	changeQianming(){
  		var dis=this.$refs.qianming.getAttribute("disabled")
  		console.log(dis)
  		if(dis=="disabled"){
  			this.$refs.qianming.removeAttribute("disabled")
  		}else{
  			this.$refs.qianming.setAttribute("disabled","disabled")
  		}
  		localStorage.setItem("qianming",this.qianming)
  	},
  	all(){
